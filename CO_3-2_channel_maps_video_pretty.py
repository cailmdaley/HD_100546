...
# Create a sin(theta) and cos(theta) array where theta is measured from
# the x-axis
...
for j in range(len(xval)):
    for i in range(len(yval)):
        ...
        sintheta[i][j] = xdisk[i][j] / \
            np.sqrt(xdisk[i][j]**2.0 + ydisk[i][j]**2.0)
        costheta[i][j] = ydisk[i][j] / \
            np.sqrt(xdisk[i][j]**2.0 + ydisk[i][j]**2.0)
...
for j in range(len(xval)):
    for i in range(len(yval)):
        ...
        if rtop[i][j] != 0.0:
            vtop[i][j] = cm2km * \
                np.sqrt(grav * mstar / rtop[i][j]) * \
                np.sin(inc) * sintheta[i][j]

        if rbot[i][j] != 0.0:
            vbot[i][j] = cm2km * \
                np.sqrt(grav * mstar / rbot[i][j]) * \
                np.sin(inc) * sintheta[i][j]

        if rflat[i][j] != 0.0:
            vflat[i][j] = cm2km * \
                np.sqrt(grav * mstar / rflat[i][j]) * \
                np.sin(inc) * sintheta[i][j]
...
for i in range(nv):
    vel[i] = (ckm * ((freq - vel[i]) / freq)) - source + 0.00000022 
...
images  = []
for i in np.arange(22,135):
# for i in np.arange(105,106):
    ...
    plt.title('Orbital Motion of Gas in Planetary System HD 100546', fontsize=20)

    bmin = head['bmin'] * 3600.
    bmaj = head['bmaj'] * 3600.
    bpa = head['bpa']
    el = ellipse(xy=[3, -3], width=bmin, height=bmaj, angle=-bpa,
                 fc='k', ec='w', fill=True, zorder=10)
    ax.add_artist(el)

    text(3.88, 3.66, velocity + ' km/s', fontsize=20, color='k')
    text(-2.0, 3.66, r'CO ($\mathrm{J}=3-2$)', fontsize=20, color='k')

    plot([0.0], [0.0], '+', markersize=8, markeredgewidth=1, color='k')

    cmap = ax.imshow(img_chan['image'][:][:][chan],
                  extent=[cxmin, cxmax, cymin, cymax],
                  vmin=cbmin,
                  vmax=np.max(img_chan['image']),
    ...
    C1 = ax.contour(xorig, yorig, 
        img_mom0['image']/line_rms,
        levels=contlev,
        linewidths=0.5,
        linestyles='dashed')
        
    # contours = ([vel[chan]])
    # ax.contour(xval,xval,
    #              flat_vel['image'],
...
# Create and add a colour bar with label and tics
    cax = fig.add_axes([0.891, 0.125, 0.05, 0.755])
    cbar = colorbar(cmap, cax=cax)
    cbar.ax.text(0.29, 0.525, 'Jy/beam', fontsize=16, rotation=90,
    path_effects=[PathEffects.withStroke(linewidth=3, foreground="w")])
    cbar.set_ticks(np.arange(0, cbmax, cbtmaj))
    minorLocator = AutoMinorLocator(cbtmaj/cbtmin)
    cbar.ax.yaxis.set_minor_locator(minorLocator)
    cbar.update_ticks()
    

# Save the figure to pdf/eps
    print('Channel_Movie/Channel{:02d}.png'.format(chan))
    # plt.show()
    fig.savefig('Channel_Movie/Channel{:03d}.png'.format(chan))
    images.append(imageio.imread('Channel_Movie/Channel{:03d}.png'.format(chan)))
imageio.mimsave('channel_movie.gif', images)
